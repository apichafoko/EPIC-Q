...
async function handler(
  ...
  try {
    ...
    const expiresIn = parseInt(searchParams.get('expiresIn') || '3600', 10); // 1 hora por defecto para previews

    // Validar acceso
    const resource = await prisma.project_resources.findUnique({
      where: { id: resourceId },
      select: { project_id: true, s3_key: true, url: true },
    });
    ...
    const signedUrl = await ResourceService.getSignedResourceUrl(resourceId, expiresIn);
